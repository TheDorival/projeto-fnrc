...
Em cada VM, crie o usuário administrador `adminifal` com senha `adminifal` e conceda privilégios de superusuário:
...
```bash
groups adminifal
# Saída esperada: adminifal : adminifal sudo
```
...
|:---|:---|:---|
| `adminifal` | Administrador do Sistema | Administrador — sudo (senha: `adminifal`) |
...
cat /etc/passwd | grep -E 'adminifal|daniel|iago|gabriel|leonardo'
